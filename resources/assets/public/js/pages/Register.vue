    ...
    <section id="register" v-if="!$global.loading" class="content-area">
        <div>
                ...
                <v-form ref="form" v-model="valid">
                    <v-container fluid grid-list-lg="true">
                        <v-layout row wrap>
                            ...
                            <v-flex xs8>
                                <v-text-field :rules="validationRules.email" :error-messages="emailErrors" required v-model="form.user.email" @change="checkEmail" label="E-mail (usado para login no sistema)">
                                </v-text-field>
                            </v-flex>
                            <v-flex xs4>
                            ...
                            <div class="programs-area">
                                <v-flex xs12>
                                    <v-layout row wrap>
                                        <v-flex v-if="(program.id == 22 && ['T', 'V'].indexOf(form.radio_type) >= 0) || 
                                                (program.id != 22 && ['F', 'A', 'W'].indexOf(form.radio_type) >= 0)" xs4 v-for="program in programsList" :key="program.id">
            ...
            <div class="btn-area clear-fix">
                <v-btn :loading="loading" class="clearfix" color="secondary" large :disabled="!valid || loading" @click="submit">Cadastrar</v-btn>
            </div>
        </div>
...
import {
    ...
    required,
    email
} from '../../../admin/js/lib/ValidationFunctions';
import MessageDialog from '../../../admin/js/components/MessageDialog2';
import {
    ufList
} from '../../../admin/js/lib/UfList';
